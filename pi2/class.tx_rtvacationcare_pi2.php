...
';
	...
	function formatAsPDF($vacation) {
		...
		// content of pdf
		$salutation = utf8_decode($this->pi_getLL('pdf.salutation'));
		...
		// go
		$pdf->tx_fpdf->template = 'fileadmin/templates/pdf/template.pdf';
		$pdf->AliasNbPages();
...
#echo t3lib_div::debug($place,'');
	    ...
	    if($media->meta['uid'] > 0 ) {
	    	$vacationImage = $img['file'] = $media->meta['file_path'].$media->meta['file_name'];
	    }     
	    $posY = $pdf->GetY();
	    $posX = 30;
	    ...
	    $posY = $pdf->GetY();
	    $posX = 55;
	    $pdf->Image('typo3conf/ext/rt_vacationcare/pi2/res/images/wer-kommt-mit-print.png',$posX,$posY,100,0); 
	    ...
	    $posY = $pdf->GetY();
	    $posX = 55;
	    $pdf->Image('typo3conf/ext/rt_vacationcare/pi2/res/images/wir-kommen-auch-print.png',$posX,$posY,100,0); 
		...
		for ($i = 0; $i <= count($allImages); $i++) {
			if ($allImages[$i]['image'] != '') {	
		    	$imageSize = getimagesize($imagePath.utf8_encode($allImages[$i]['image']));
				if ($imageSize[0] > $imageSize[1]) {
					// quer
					$pdf->Image($imagePath.utf8_encode($allImages[$i]['image']),$posX,$posY,0,30);
					$format = $imageSize[0]/30;
					$newWidth = $imageSize[1]/$format+10;
					$posX += $newWidth+20;
				} else {
					// hoch
					$pdf->Image($imagePath.utf8_encode($allImages[$i]['image']),$posX,$posY,20,0);
					$posX += 30;
				}
		...
		// info text
 		//
 		if ($vacation['info2'] || $vacation['info3']) {
 			$pdf->AddPage();
 			$pdf->MultiCell(0,5, utf8_decode($vacation['info2']));
 			$pdf->Ln();
 			$pdf->MultiCell(0,5, utf8_decode($vacation['info3']));
 		}
	...
	function getMobilePhone($vId) {
		...
		if ($chiefRes) {
			$chief = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($chiefRes);
			$chief = $chief['uid'];
			$theChief = $this->pi_getRecord('tt_address', $chief);
			$out .= $theChief['mobile'];
		}
		return $out;
	}
...
class myPDF extends PDF { 
	...
	function peopleTable($header, $data, $vacation) {
		...
		$candleImage = 'typo3conf/ext/rt_vacationcare/pi2/res/images/kerze.jpg';
