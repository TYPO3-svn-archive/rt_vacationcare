...
class  tx_rtvacationcare_module2 extends t3lib_SCbase {
				...
				function moduleContent() {
					...
					$postData = t3lib_div::_POST('setNewCaretaker');
echo t3lib_div::debug($postData,'');
		...
		#echo t3lib_div::debug($delConf,'');
								if ($delConf > 0 && $vacId > 0) {
									$delConf = $GLOBALS['TYPO3_DB']->exec_DELETEquery('tx_rtvacationcare_vacations_caretaker_mm', 'uid_foreign = '.$delConf.' AND uid_local = '.$vacId);
								}
...
#echo t3lib_div::debug($registeredAttendees,'');
					...
					// listview table
					$kcont .= '<h2>'.$LANG->getLL('regTitle').' '.$LANG->getLL('for').' '.$vacation['title'].'</h2>';
					// back button
					$kcont .= '<a href="index.php?SET[function]=1">'.$LANG->getLL('backToListview').'</a>';
					$kcont .= $this->doc->spacer(10);
...
*/
					...
					while($allVacations = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($vacationRes)) {
						...
						// number and title
						$kcont .= '<tr><td>'.$allVacations['nr'].'</td><td style="color:'.$cellColour.'">'.$allVacations['title'].'</td>';
						// date
...
#echo t3lib_div::debug($caretaker,'');
					...
					$out .= '<select name="setNewCaretaker" size="10" style="width:250px;" multiple>';
